  ...
  <xsl:template match="gmd:extent" mode="mcpextent">
    <xsl:element name='gex:extent'>
      ...
      <xsl:element name='gex:EX_Extent'>
        ...
        <xsl:apply-templates select="node()" mode="from19139to19115-3"/>
      </xsl:element>
    </xsl:element>
  </xsl:template>

  <xsl:template match="mcpold:EX_TemporalExtent" mode="from19139to19115-3">
    <gex:EX_TemporalExtent>
      <!-- TODO: Where to put mcpold:currency and mcpold:temporalAggregation? 
           Does anyone use them? -->
      <xsl:apply-templates select="@*[local-name()!='isoType']|*[namespace-uri()!='http://schemas.aodn.org.au/mcp-2.0']" mode="from19139to19115-3"/>
    </gex:EX_TemporalExtent>
  </xsl:template>
</xsl:stylesheet>
